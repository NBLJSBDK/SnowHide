...
val versionHistory = listOf(
    VersionRecord(
        "v0.6.0",
        "流畅度与无障碍服务稳定性",
        "统一主屏、文件夹、搜索回主屏、Dock 回弹和冻结滤镜等显式动画为最快连续动画；限制图标加载范围和并发，将列表派生、状态缓存序列化等工作移出主线程，并合并批量状态发布；优化 Pager 滑动吸附阈值，让短滑和快速连续滑动更顺滑。修复无障碍服务初始化异常导致进程退出、系统重绑定未重新初始化和动态广播未清理的问题；服务断开后可安全重绑定，Recent 控制器后台异常不再杀掉服务。RMX3888 覆盖安装及禁用/恢复服务重绑定验证通过，保持 versionCode=1、Backup v1、Shizuku 执行链和应用数据安全边界不变。",
    ),
    VersionRecord(
        "v0.5.4",
        "桌面快捷方式管理与残留记录修复",
        "新增快捷方式管理页，保留冻结全部、启用全部、智能清理、快速启停开关和打开雪藏五个预留入口；新增“清理全部快捷方式”，通过 Shizuku 清除系统保存的快捷方式记录并恢复雪藏长按菜单，修复 Realme Launcher 保留失效记录导致知乎等快捷方式无法再次创建的问题。清理操作不删除雪藏宫格、应用数据或应用本身；保持 versionCode=1、Backup v1、Shizuku 执行链和应用数据安全边界不变。",
    ),
    VersionRecord(
        "v0.5.3",
        "应用分身徽标与快捷方式视觉优化",
