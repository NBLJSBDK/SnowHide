...
- `versionName` = 语义版本（当前正式版 `0.4.1`，上一正式版 `0.4.0`）；编译时间由 gradle 单独生成并展示，不能把编译时间混入版本号。
...
9. 应用桥（已完成调研，优先级最后）
10. 批量停用/启用及 Dock 上滑停用的耗时和卡顿：在不改变逐包进度、安全边界和 Binder IO 线程规则的前提下测量并优化
11. 批量进度条的 Edge-to-Edge/挖孔适配：复现顶部摄像头区域黑边，以及横屏时摄像头区域与宫格栏颜色不一致

**当前正式版（`0.4.1`，已完成 Release 编译、安装和用户确认）**：
- 美化设置新增动画速度四档滑竿：关 / 高 / 中 / 低，默认中。
- 应用显式导航、重进、搜索回主屏、Dock 回弹和冻结滤镜遵循动画档位；关闭时直接切换。
- Backup v1 新增 `animation_level`，并兼容旧 `animations_enabled` 字段。
- 宫格设置统一入口：主屏和任意文件夹空白处长按均可打开。
- 宫格设置统一为「布局设置 / 美化设置 / 目录设置」；美化设置增加显示图标名称。
- 目录设置增加循环滑动、排除文件夹、显示返回主屏按钮和重进时回到主屏。
...
8. **批量操作性能待调查**：用户反馈“全部停用/启用”和 Dock 上滑停用存在耗时或卡顿，后续先采样 Binder、命令和 UI 等待时间，再决定是否优化。
...
- 当前分支：`master`，当前正式发布提交：本次 `v0.4.1` 发布提交；上一正式发布提交：`309c05d`（`v0.4.0` tag）；历史回退基线：`5cd8ce6`（`baseline-before-v0.2.0` tag）。
- `v0.4` 基线 Tag 已指向 `666eae5`（主线开始 `0.4.0-dev` 开发）；开发阶段不创建 `-dev` Tag。
- `v0.4.0` 正式发布提交 `309c05d` 已完成，`master` 分支和 `v0.4.0` Tag 已推送。
- `v0.4.1` 已完成 Release 编译、覆盖安装和用户确认；正式发布提交将在本次版本存档后补入上方记录。
