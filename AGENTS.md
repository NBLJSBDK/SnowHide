...
- 上划释放后立即从 Dock 隐藏目标图标；冻结失败自动恢复，成功立即更新共享状态并继续后台真实状态校准。
- 同一应用的重复冻结请求会被忽略；冻结仍经既有 FreezeUseCase/PowerEngine 链路执行。
- 继承 `0.4.1` 的四档动画速度、主屏/文件夹导航动画和 Backup v1 兼容规则。
...
- 当前分支：`master`，当前正式发布提交：本次 `v0.4.2` 发布提交；上一正式发布提交：`9897d84`（`v0.4.1`）；历史回退基线：`5cd8ce6`（`baseline-before-v0.2.0` tag）。
...
- `v0.4.2` Dock 跟手与即时反馈修复已完成 Release 验收，本次发布提交和 Tag 正在存档。
