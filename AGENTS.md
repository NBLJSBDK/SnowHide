...
- `0.5.0` 已完成正式发布；当前开发目标为 `0.5.1`，开发阶段使用 `0.5.1-dev`，正式版本号和 `VersionHistory.kt` 只在发布存档阶段更新。
...
**当前开发阶段（`0.5.1-dev`）**：
- 从主屏或文件夹宫格应用的长按菜单请求创建桌面快捷方式；快捷方式以完整 `AppTarget`（包名 + 用户空间）为唯一目标，分身不回退到 user 0。
- 快捷方式优先使用当前选中的图标包图标，并在右下角合成雪藏小图标；点击行为与宫格单击一致，冻结目标先临时解冻再启动。
