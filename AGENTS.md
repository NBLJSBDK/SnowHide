...
- `versionName` = 语义版本（当前正式版 `0.5.1`，上一正式版 `0.5.0`）；编译时间由 gradle 单独生成并展示，不能把编译时间混入版本号。
- `0.5.1` 已完成正式发布；下一开发目标待确定，开发阶段使用目标版本加 `-dev` 后缀，正式版本号和 `VersionHistory.kt` 只在发布存档阶段更新。
...
**当前正式版（`0.5.1`，2026-09-05，已完成 Release 编译、签名和自动化验证；真机行为沿用已安装验收的 `0.5.1-dev`，正式包因设备断开未重复覆盖安装）**：
...
**下一开发阶段（版本目标待定）**：
- `0.5.1` 功能已进入正式版；下一开发目标和版本号待后续确认。
...
- 当前分支：`master`，当前正式发布提交：本次 `0.5.1` 发布提交；上一正式发布提交：`e872da5`（`v0.5.0`）；历史回退基线：`5cd8ce6`（`baseline-before-v0.2.0` tag）。
...
- `v0.5.1` 正式发布提交为本次发布提交，Tag 创建后保持不移动。
