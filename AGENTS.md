...
7. **Tag 后 APK 归档**：每次创建正式 Tag 后，将对应的 Release/Debug APK 分别复制到 `assets/release/SnowHide-vX.Y.Z-release.apk` 和 `assets/debug/SnowHide-vX.Y.Z-debug.apk`；`assets/` 保持本地忽略，不提交二进制到 Git。
...
- `versionName` = 语义版本（当前正式版 `0.6.0`，上一正式版 `0.5.4`）；编译时间由 gradle 单独生成并展示，不能把编译时间混入版本号。
- `0.6.0` 已于 2026-09-06 完成正式发布；下一开发目标待确定。正式版本号和 `VersionHistory.kt` 只在发布存档阶段更新。
...
- v0.5.2：调整布局默认值为主屏图标 48dp、Dock 图标 48dp、操作图标 24dp、上下间距 0 和文件夹预览 2×2；修复 Dock 图标形状不跟随圆形设置，以及美化设置显示图标包包名的问题。
- v0.5.4：增加快捷方式管理页和五个预留固定入口；增加“清理全部快捷方式”，通过 Shizuku 清除系统快捷方式记录并恢复动态长按菜单，修复 Realme Launcher 残留记录导致快捷方式无法再次创建的问题。
- v0.6.0：统一显式动画为最快连续动画并优化图标加载、列表派生、状态缓存和批量状态发布；降低 Pager 滑动吸附阈值；修复无障碍服务初始化失败、系统重绑定恢复和动态广播清理问题，Recent 后台异常不再杀掉服务。

**当前正式版（`0.6.0`，2026-09-06，已完成 Release 编译、签名、覆盖安装和真机验证）**：
- 统一主屏、文件夹、搜索回主屏、Dock 回弹和冻结滤镜等显式动画为最快连续动画；限制图标加载到当前稳定页面并控制 IO 并发，减少主线程列表派生和状态缓存序列化工作，合并批量状态发布。
- 降低 Pager 滑动吸附阈值，短滑和快速连续滑动更顺滑；RMX3888 已验证短滑、连续正反向快滑和重绑定后的服务状态。
- 将无障碍服务声明改为系统可绑定，延长 ColorOS 连接状态宽限期；初始化失败不再直接杀掉服务，系统断开/重绑定会重新初始化控制器和动态广播，Recent 后台异常被隔离并记录。
- Release APK 已在 RMX3888 覆盖安装；通过仅禁用/恢复雪藏服务的重绑定测试，未清除数据、未卸载、未重启。原始“只有重启后恢复”的故障本次未再次复现。
- `testDebugUnitTest`、`assembleRelease` 和 `git diff --check` 已通过；保持 `versionCode=1`、Backup v1、Shizuku 执行链和应用数据安全边界不变。

**0.5.4 发布回顾（2026-09-05）**：
- 新增“快捷方式管理”页面，保留冻结全部、启用全部、智能清理、快速启停开关和打开雪藏五个预留入口。
- 新增“清理全部快捷方式”，清除系统保存的全部快捷方式记录并恢复雪藏长按菜单；不删除雪藏宫格、应用数据或应用本身。
- 修复 Realme Launcher 保留失效 pinned shortcut 记录后，知乎等桌面快捷方式无法再次创建的问题。

**0.5.2 发布回顾（2026-09-05）**：
- 默认布局使用主屏图标 48dp、Dock 图标 48dp、操作图标 24dp、上下间距 0 和文件夹预览 2×2；已有用户手动保存的设置不被覆盖。
- Dock 图标现在遵循圆形或圆角方形设置，未被图标包覆盖的系统回退图标也使用相同形状。
- 美化设置的图标包摘要显示图标包名称，不再直接显示包名；图标包选择列表行为保持不变。

**0.5.1 发布回顾（2026-09-05，已完成 Release 编译、签名和自动化验证；正式包随后已在 `192.168.9.35:38589` 覆盖安装并确认 `versionName=0.5.1`）**：
- 从主屏或文件夹宫格长按菜单创建桌面快捷方式，使用完整 `AppTarget` 区分主应用和分身；快捷方式优先使用当前图标包和图标形状，叠加雪藏徽标与分身用户徽标。
- 快捷方式点击复用宫格单击行为：目标仍在宫格中且已冻结时先临时解冻，再按用户空间启动主应用或分身；主应用使用包管理器启动入口，分身使用 `LauncherApps` 启动入口。
- 目标校验和临时解冻策略收口到 `domain/shortcut`，由组合根注入平台适配；增加 JVM 回归测试，保持 `versionCode=1`、Backup v1、Shizuku 执行链和应用数据安全边界不变。

**0.5.0 发布回顾（2026-09-03，已完成 Release 编译和自动化验证；应用分身行为沿用 2026-09-01 用户确认）**：
...
**下一开发阶段（版本号待确定）**：
- `0.6.0` 已完成流畅度优化和无障碍服务稳定性修复；后续开发目标待用户确定。
...
- 当前分支：`master`，当前正式发布提交：本次 `0.6.0` 发布提交；上一正式发布提交：本次 `0.5.4` 发布提交（`v0.5.4`）；历史回退基线：`5cd8ce6`（`v0.1.7` tag，`v0.2.0` 开发前基线）。
...
- `v0.5.1` 正式发布提交为 `6257fea`，Tag 创建后保持不移动。
- `v0.5.2` 正式发布提交为本次发布提交，Tag 创建后保持不移动。
- `v0.5.3` 正式发布提交为 `b1bb261`，Tag 创建后保持不移动。
- `v0.5.4` 正式发布提交为本次发布提交，Tag 创建后保持不移动。
- `v0.6.0` 正式发布提交为本次发布提交，Tag 创建后保持不移动；正式 Release/Debug APK 分别归档为 `assets/release/SnowHide-v0.6.0-release.apk` 和 `assets/debug/SnowHide-v0.6.0-debug.apk`。
