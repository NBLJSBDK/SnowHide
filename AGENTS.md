...
- `versionName` = 语义版本（当前正式版 `0.4.0`，上一正式版 `0.3.2`）；编译时间由 gradle 单独生成并展示，不能把编译时间混入版本号。
...
**当前正式版（`0.4.0`，已完成 Release 编译、安装和用户确认）**：
...
- 当前分支：`master`，当前正式发布提交：`309c05d`（`v0.4.0` tag）；上一正式发布提交：`b206ae1`（`v0.3.2` tag）；历史回退基线：`5cd8ce6`（`baseline-before-v0.2.0` tag）。
