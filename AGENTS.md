...
- `versionName` = 语义版本（当前正式版 `5.0`，上一正式版 `0.4.2`）；编译时间由 gradle 单独生成并展示，不能把编译时间混入版本号。
- `5.0` 已完成正式发布；下一开发目标待确定，开发阶段仍使用目标版本加 `-dev` 后缀，正式版本号和 `VersionHistory.kt` 只在发布存档阶段更新。
...
**当前正式版（`5.0`，2026-09-01，已完成 Release 编译、安装和用户确认）**：
...
- 当前分支：`master`，当前正式发布提交：`b896e2c`（`v5.0`）；上一正式发布提交：`4a20b4d`（`v0.4.2`）；历史回退基线：`5cd8ce6`（`baseline-before-v0.2.0` tag）。
...
- `v5.0` 正式发布提交为 `b896e2c`，Tag 创建后保持不移动。
