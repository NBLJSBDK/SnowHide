...
- `versionName` = 语义版本（当前正式版 `0.5.4`，上一正式版 `0.5.3`）；编译时间由 gradle 单独生成并展示，不能把编译时间混入版本号。
- `0.5.4` 已完成正式发布；当前开发目标为 `0.6.0`，开发阶段使用 `0.6.0-dev`，正式版本号和 `VersionHistory.kt` 只在发布存档阶段更新。
...
**当前正式版（`0.5.4`，2026-09-05，已完成 Release 编译、签名、覆盖安装和真机验证）**：
...
**下一开发阶段（`0.6.0` 流畅度优化）**：
- 当前开发版号为 `0.6.0-dev`，优先优化宫格启动、滑动、点击和冻结反馈的流畅度；不新增非必要功能。
- 第一轮先停止未打开页面的全量列表派生、限制图标加载并发、修复图标缓存并发访问、把状态缓存序列化移出主线程，并合并批量状态发布；美化设置移除动画四档滑杆，所有操作固定使用最快的连续动画。同步修正无障碍服务声明为系统可绑定、支持断开后的重绑定，并延长 ColorOS 连接状态宽限期。
...
- 当前分支：`master`，当前正式发布提交：本次 `0.5.4` 发布提交；上一正式发布提交：`b1bb261`（`v0.5.3`）；历史回退基线：`5cd8ce6`（`v0.1.7` tag，`v0.2.0` 开发前基线）。
...
- `0.6.0-dev` 为当前开发阶段版号，不创建开发 Tag；正式版号、更新记录和正式 Tag 等功能验收后再单独存档。
